...
const CompRanks = ({floor, isFirstHalf, isFilterActive, checked}) => {
    ...
    const checkIncluded = (comp) => {    
        ...
        return included.every( includedChar=> 
            (!includedChar || includedChar.name === "None") || compCharacterNames.find(compChar=>compChar===includedChar.name)
         )
    }
    ...
    let checkExcluded = (comp) => {
        ...
        for (let compCharName of compCharacterNames) {
            if ( excludedCharacters.includes(compCharName) ){
                return true;
            }
        }
    }
    ...
    let comps = [];
    if(data) {
        comps = data.slice(0, numTeams).filter(comp =>
            isFilterActive && checkIncluded(comp) && !checkExcluded(comp))
    } else {
